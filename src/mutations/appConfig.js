  ...
  return xhr(replaceUrlPlaceholders(urlAppConfig))
    .then((res) => {
      try {
        ...
        const currentBuild =
					parseInt(
					  getCookie(`${process.env.PREFIX}_build`).replace(/\./gim, '')
					) || parseInt(process.env.BUILD_NO.replace(/\./gim, ''));
        const expectedBuild = parseInt(res.web.replace(/\./gim, ''));
        const cookieReload = parseInt(getItemWithExpiry('reload') || '0');
              ...
              cbOk: () => {
                ...
                window.open(
                  process.env.PLATFORM === 'ios'
                    ? getConfigByKey('appStore')
                    : getConfigByKey('playStore'),
                  '_blank'
                );
              },
            });
