...
const eventTags = ['INTERESTED', 'GOING', 'NOT_INTERESTED'];
  ...
  constructor(props) {
	...
	renderEventsDropdown = (item, isLastItem) => {
	  ...
	  if (eventDropdown && eventDropdown.show && eventDropdown.id === item.id) {
	    return (
	      <div
	        className={`${eventDropdown.tagged === 'INTERSTED'} ${
						style.selectEventDropdown
					} ${isLastItem ? style.lastItem : ''}`}
	      >
	        {eventTags.map((tag) => (
	          <a
	            className={`event-tag-${tag
	              .toLocaleLowerCase()
	              .replace(/_/g, '-')} ${
								eventDropdown.tagged === tag ? 'extraBold' : ''
							}`}
	            onClick={() => {
	          ...
	          >
	            <ImageLoader
	              src={`assets/images/${tag}-${
									eventDropdown.tagged === tag ? 'pink' : 'dark'
								}.png`}
	  ...
	  if (data && data.length) {
	    return data.map((i, index) => (
	      <div className={`${style.contentItem} ${style.eventItem}`}>
	        {!getCurrentUrl().includes('community') && i.community && (
	          ...
	          <div className={style.detailContent}>
	            ...
	            {i.likeCount || i.shareCount ? (
	              <div className={style.detailCount}>
	                {i.likeCount && (
	                  <span>{`${i.likeCount} ${getTranslation('LIKES')}`}</span>
	                )}
	                {i.shareCount && (
	                  <span>{`${i.shareCount} ${getTranslation('SHARES')}`}</span>
	                )}
	              </div>
	            ) : null}
	          </div>
	        ...
	        {!this.props.hideTag && (
	          <div className={style.buttons}>
	            <a
	              className={`event-tag-options ${
									i.tagged ? `extraBold ${style.buttonLikeActive}` : ''
								}`}
	              id={i.id}
	              onClick={() => {
	            ...
	            >
	              <ImageLoader
	                src={
	                  !i.tagged
	                    ? 'assets/images/INTERESTED-dark.png'
	                    : `assets/images/${i.tagged}-pink.png`
	                }
	                style={{ container: style.likeButton }}
	              />
	            ...
	            <a
	              className={`event-share ${
									i.shared ? style.buttonShareActive : ''
								}`}
	              onClick={() => {
	                this.onShareEvent(i);
	              }}
	            >
	              <ImageLoader
	                src={
	                  !i.shared
	                    ? 'assets/images/share_icon_lite.png'
	                    : 'assets/images/share_icon_dark.png'
	                }
	...
	render = ({ data, onClickItemCallback }) => {
	  return <div className={style.eventsListWrap}>{this.renderDom(data)}</div>;
	};
}
