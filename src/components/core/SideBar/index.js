	...
	render = ({ toggleSideBar, isOpen, authUser, page, communityInfo }) => {
	  ...
	  return (
	    <div className={style.sideBarContainer}>
	      ...
	      <div
	        className={
	          isOpen ? `${style.sideBar} ${style.toggled}` : style.sideBar
	        }
	      >
	        ...
	        <div className={style.sUser}>
	          ...
	          <div>
	            <div>
	              <p className={`extraBold ${style.sName}`}>{`${getTranslation(
	                'HI_NAME'
	              ).replace('{NAME}', authUser.profile.fname)}`}</p>
	              <p className={style.sLogin}>{`${getTranslation(
	                'LAST_LOGIN'
	              ).replace(
	                '{DATE_TIME}',
	                dateLastLoginFormat(authUser.loginDate)
	              )}`}</p>
	            </div>
	          </div>
	        </div>
	            ...
	            <div>
	              ...
	              {!communityInfo.data && (
	                <Link
	                  ...
	                  className={`${style.sMItem} ${style.subItem}`}
	                  onClick={this.onClickMenu}
	                >
	                  {getTranslation('SETUP_PAGE')}
	                </Link>
	              )}
	              <Link
	                id="sm-postcontent"
	              ...
	              <Link
	                ...
	                onClick={this.onClickMenu}
	              >
	                {getTranslation('PAGE_MANAGE_PAGE')}
	              </Link>
	              <Link
	                id="sm-postvolunteerannouncement"
	                href={`/post-volunteer-announcement`}
	                className={`${style.sMItem} ${style.subItem}`}
	                onClick={this.onClickMenu}
	              >
