    ...
    const data = appLandingConfig.data;
	  return (
	    <div className={style.contentContainer}>
	      ...
	      <div
	        className={isOpen ? `${style.comsAndKit} ${style.toggled}` : style.comsAndKit}
	      >
	        {/* Communities and Kit Content Starts Here */}
          <div className={style.communities}>
            <p className={`extraBold ${style.title}`}> {getTranslation('COMMUNITIES')} </p>
            <div className={style.community}>
              ...
              <div className={style.communityContent}>
                ...
                <p className={`bold`}> {data.event1.date ? data.event1.date : null} </p>
            ...
            <div className={style.community}>
              ...
              <div className={style.communityContent}>
                ...
                <p className={`bold`}> {data.event2.date ? data.event2.date : null} </p>
                <p> {data.event2.body}{data.event2.body.length > 98 ? "..." : ""} </p>
              </div>
            </div>
            <div>
              <p className={`extraBold ${style.title}`}> {getTranslation('KAKAMPINK_KIT')} </p>
              <Carousel
                showArrows={false}
                showStatus={false}
                showThumbs={false}
