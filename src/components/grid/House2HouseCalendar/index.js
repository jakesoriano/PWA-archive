...
import { FormGroup, LoaderRing, FormDropdown, ImageLoader } from '_components/core';
import {
  getRegions,
  getProvince,
  getMunicipality,
...
class House2HouseCalendar extends Component {
  constructor(props) {
    super(props);
    this.state = {
      dates: [new Date(), new Date(new Date().getTime() + (7*24*60*60*1000))],
    ...
    if (this.state.region.value !== value) {
      ...
      fetchHouse2HouseCalendar(null, filter).then(() => {
  ...
  renderModalContent = (loc, city, name, num) => (
    <div className={style.modalContent}>
      <div className={style.modalHead}>
        <p className={`bold ${style.title}`}>{getTranslation('CONTACT_CHAPTER_HEAD')}</p>
        <p className={style.location}>{`${getTranslation('MEETUP_LOCATION')}: ${loc || ''}`}, <span className={style.city}>{city}</span></p>
      </div>
  ...
  renderResults = (data) => {
    if (data && data.length) {
        ...
        <div className={style.resultItem}>
          <div className={style.head}>
            ...
            <div className={style.info}>
              ...
              <div className={style.infoText}>
                { item.date && <p className={`bold ${style.date}`}>{dateEventFormat(item.date)}</p> }
                { item.location && <p className={style.location}>{item.location}, <span className={style.city}>{item.city.replace('CITY OF', '')}</span></p> }
                { item.contact?.name && <p className={style.name}>{`${getTranslation('CONTACT_PERSON')}: ${item.contact?.name}`}</p> }
              </div>
            </div>
          </div>
          ...
          <div className={style.footer}>
            <button
              ...
              onClick={() => {
                componentModal({
                  content: this.renderModalContent(item.location, item.city.replace('CITY OF', ''), item.contact?.name, item.contact?.mobile),
                  transparentBG: true,
      ...
      return (
        <p className={style.noResult}>{getTranslation('NO_DATA')}</p>
      )
    }
  };
  ...
  render = ({ h2hcalendar },
    ...
    <div className={style.h2hcWrap}>
      ...
      <p className={`semiBold ${style.message}`}>{getTranslation('RESULTS_AUTOMATIC')}</p>
      <div className={style.results}>
        <p className={`bold ${style.heading}`}>{getTranslation('RESULTS')}</p>
        {/* results */}
        {this.renderResults(h2hcalendar?.data)}
        {/* show more */}
        {h2hcalendar && h2hcalendar.data.length < h2hcalendar.total && !h2hcalendar.fetching && (
