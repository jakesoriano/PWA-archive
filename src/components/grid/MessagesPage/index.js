...
class MessagesPage extends Component {
  constructor(props) {
	...
	viewMessage = (feedId, listingId) => {
	  route(
			`${this.props.page}/messages-chat?feedId=${feedId}&listingId=${listingId}`
	  );
	};
	componentDidMount = () => {
	  fetchMessages();
	  ...
	  <div className={style.messagesWrap}>
	    {messages.data.map((item, i) => (
	      <div
	        ...
	        onClick={() => this.viewMessage(item.feedId, item.listingId)}
	      >
	        <ImageLoader
	        ...
	        />
	        <div
	          className={`${style.details} ${item.unread ? style.unread : ''}`}
	        >
