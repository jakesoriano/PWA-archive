  ...
  constructor(props) {
	...
	componentDidMount = () => {
	  if (this.props.leaderEditPost) {
	    ...
	    if (this.props.leaderEditPost.type === 'events') {
	      editData = this.props.leaderCommunityEvents.data.find(
	        (e) => e.id === this.props.leaderEditPost.id
	      );
	    } else {
	      editData = this.props.leaderCommunityAnnouncements.data.find(
	        (e) => e.id === this.props.leaderEditPost.id
	      );
	    }
	    this.setState({
	      contentType: {
	        value:
						this.props.leaderEditPost.type === 'events'
						  ? 'Event'
						  : 'Announcement',
	        error: '',
	        message: '',
	...
	submitEditData = (image) => {
	  ...
	  if (this.props.leaderEditPost.type === 'events') {
	    ...
	    updateCommunityEvent({ data }, this.props.leaderEditPost.id)
	      .then((res) => {
	        displayPageLoader(false);
	        if (res && res.success) {
	          this.setState({
	    ...
	    updateCommunityNews({ data }, this.props.leaderEditPost.id)
	      .then((res) => {
	        displayPageLoader(false);
	        if (res && res.success) {
	          this.setState({
	...
	handleContinue = () => {
	  if (!this.state.contentType.value) {
	  ...
	  } else if (this.state.contentType.value === 'Event') {
	    if (
	    ...
	    } else if (this.props.leaderEditPost) {
	      ...
	      <div className={style.infoWrap}>
	        <FormGroup label={getTranslation('IMAGE')}>
	          <div className={style.attachmentWrap}>
	            <div
	              className={`${style.attachmentInputWrap} 
								${
									typeof this.state.attachment.file === 'string' ||
									this.state.attachment.file instanceof String
									  ? style.hideInput
	...
	render = ({ authUser }, { contentType, contentTypeOptions }) => {
	  ...
	  return (
	    <div className={style.postContent}>
	      <div className={style.infoWrap}>
	        <FormGroup
	          label={getTranslation('CONTENT_TYPE')}
	          hasError={this.state.contentType.hasError}
	        >
	          <FormDropdown
