  ...
  constructor(props) {
	...
	componentDidUpdate = () => {
	  if (
	    this.state.moreFetching &&
			...
			!this.getSelectedTabContent().fetching
	  ) {
	    this.setState({
	      moreFetching: false,
	    });
	...
	handleShowMore = () => {
	  if (!this.state.moreFetching) {
	    ...
	    // fetch
	    if (active === 'news') {
	      getCurrentUrl() === '/community-details'
	        ? fetchNewsByCommunity(
	          fetchNews(
	            this.props.communityDetails.details.id,
	            this.getSelectedTabContent().page + 1
	          )
					  )
	      ...
	      getCurrentUrl() === '/community-details'
	        ...
	        : fetchEvents(this.getSelectedTabContent().page + 1);
	    } else {
	...
	renderTabs = (tabTitle) => {
	  return this.state.tabs.map((i) => {
	    return (
	      ...
	      >
	        {(tabTitle && getTranslation(tabTitle[i])) ||
						getTranslation(i.toUpperCase())}
	      </span>
	    );
	  });
	};
	    ...
	    <>
	      <div className={style.newsAndEvents}>
	        ...
	        <div className={style.content}>
	          ...
	          {this.getSelectedTabContent() &&
						this.getSelectedTabContent().page === 1 &&
						!this.getSelectedTabContent().data.length &&
						this.getSelectedTabContent().fetching ? (
	          ...
	          {state.active &&
							this.getSelectedTabContent() &&
							this.getSelectedTabContent().data.length <
								this.getSelectedTabContent().total &&
							!this.getSelectedTabContent().fetching && (
	            <button
