...
class HomeFeatures extends Component {
  constructor(props) {
	...
	onNextGuide = () => {
	  this.setState({
	    activeGuide:
				this.state.activeGuide === 4 ? 0 : this.state.activeGuide + 1,
	  });
	    ...
	    return (
	      <div className={style.tooltip}>
	        ...
	        <p className={`bold ${style.title}`}>
	          {getTranslation(`TOUR_GUIDE${item}_TITLE`)}
	        </p>
	        <p className={style.desc}>
	          {getTranslation(`TOUR_GUIDE${item}_DESC`)}
	        </p>
	      </div>
	    );
	  }
	    ...
	    <div className={style.body}>
	      ...
	      <a
	        ...
	        className={`${style.item} ${
						this.isItemActiveGuide(1) ? style.activeGuide : ''
					}`}
	      >
	        ...
	        <p className={style.description}>{`${members?.data?.length} ${
						members?.data?.length === 1
						  ? getTranslation('MEMBER')
						  : getTranslation('MEMBERS')
					}`}</p>
	        <a className={`bold ${style.button}`}>
	          {getTranslation('INVITE_OTHERS')}
	        </a>
	        {/* tooltip */}
	      ...
	      <a
	        ...
	        className={`${style.item} ${
						this.isItemActiveGuide(2) ? style.activeGuide : ''
					}`}
	      >
	        <div className={style.iconWrap}>
	          <ImageLoader
	        ...
	        <p className={`${style.description} lowercase`}>{`${
						tasks.data
						  ? this.taskCompletedCount() + '/' + tasks?.data?.length
						  : 0
					} ${getTranslation('COMPLETED')}`}</p>
	        <a className={`bold ${style.button}`}>
	          {getTranslation('DAILY_TASKS')}
	        </a>
	        {/* tooltip */}
	      ...
	      <a
	        ...
	        className={`${style.item} ${
						this.isItemActiveGuide(3) ? style.activeGuide : ''
					}`}
	      >
	        <div className={style.iconWrap}>
	          <ImageLoader
	        ...
	        <p className={style.description}>
	          {getTranslation('HF_VOLUNTEER_DESC')}
	        </p>
	        <a className={`bold ${style.button}`}>
	          {getTranslation('VOLUNTEER_KIT')}
	        </a>
	        {/* tooltip */}
	      ...
	      <a
	        ...
	        className={`${style.item} ${
						this.isItemActiveGuide(4) ? style.activeGuide : ''
					}`}
	      >
	        <div className={style.iconWrap}>
	          <ImageLoader
	        ...
	        <p className={style.description}>
	          {getTranslation('HF_COMMUNITY_DESC')}
	        </p>
	        <a className={`bold ${style.button}`}>
	          {getTranslation('JOIN_COMMUNITIES')}
	        </a>
	        {/* tooltip */}
